...
//        crditPrintHeaderLayout1.visibility= View.VISIBLE
    ...
    private fun getIntentData() {
        if (intent.getParcelableExtra<Invoice>(IE_INVOICE) != null) {
            invoice = intent.getParcelableExtra(IE_INVOICE)
        }
        if (intent.getParcelableArrayListExtra<SoldProductInfo>(IE_SOLD_PRODUCT_LIST) != null){
        ...
        if (intent.getStringExtra(IE_PRINT_MODE) != null){
        ...
        for (i in promotionList.indices) {
            ...
            for ((indexForNew, promotion) in newPresentList.withIndex()) {
                ...
                if (stockId != stockId1 && (indexForNew + 1) == newPresentList.size) {
                    val newPromotion =
                        Promotion() // Check currency_id, price, promoPlanId, promotion price, product name

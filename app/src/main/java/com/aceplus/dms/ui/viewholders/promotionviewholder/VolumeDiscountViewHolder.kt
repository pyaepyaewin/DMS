...
class VolumeDiscountViewHolder (itemView: View): BaseViewHolder<VolumeDiscountDataClass>(itemView)
{
    override fun setData(data: VolumeDiscountDataClass) {
        ...
        itemView.row_vd_item_discount_from.text = data.from_sale_amount.toDouble().toInt().toString()
        itemView.row_vd_item_discount_to.text = data.to_sale_amount.toDouble().toInt().toString()

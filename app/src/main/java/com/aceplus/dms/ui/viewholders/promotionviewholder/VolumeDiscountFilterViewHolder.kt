...
class VolumeDiscountFilterViewHolder(itemView: View): BaseViewHolder<VolumeDiscountFilterDataClass>(itemView) {
    override fun setData(data: VolumeDiscountFilterDataClass) {
        itemView.row_vd_filter_from_date.text=data.start_date.substring(0, 10)
        itemView.row_vd_filter_to_date.text=data.end_date.substring(0, 10)
        ...
        itemView.row_vd_filter_group.text=data.name
        ...
        if (data.exclude != null) {
            if (data.exclude == "0") {
                itemView.row_vd_filter_exclude.text = "No"
            } else {
                itemView.row_vd_filter_exclude.text = "Yes"
            }
        }

...
interface DeliveryItemDao {
    ...
    fun deliveryItemData(deliveryId:String): List<DeliveryItem>

    @Query("update delivery_item set received_quantity =  :soldQty, order_quantity = (order_quantity - :soldQty) where delivery_item.stock_id = :stockID")
    fun updateDeliveryItemQty(soldQty: Double, stockID: String)
    ...
    @Query("select * from delivery_item where stock_id = :stockId")
    fun deliveryItemOnlyData(stockId:String):DeliveryItem
}
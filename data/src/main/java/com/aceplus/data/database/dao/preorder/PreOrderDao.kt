...
interface PreOrderDao {
    ...
    @Query("select invoice_id,customer_name,address,discount,advance_payment_amount,net_amount from pre_order inner join customer on customer.id = pre_order.customer_id where date(pre_order.pre_order_date) between date(:fromDate) and date(:toDate) ")
    fun getSalesOrderHistoryDateFilterList(fromDate:String,toDate:String): List<SalesOrderHistoryFullDataReport>
    ...
    @Query("select * from pre_order WHERE  customer_id BETWEEN :fromCusNo AND :toCusNo and date(pre_order_date) = date(:newDate)")
    fun getSaleVisitForPreOrder(fromCusNo: Int, toCusNo: Int, newDate: String): List<PreOrder>

    @Query("select * from pre_order where date(pre_order_date) = date(:now) and advance_payment_amount > 0")
    fun getPreOrderListForEndOfDay(now: String): List<PreOrder>?

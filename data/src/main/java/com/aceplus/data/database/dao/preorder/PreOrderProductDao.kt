...
interface PreOrderProductDao {
    ...
    @Query("select pop.id, pop.sale_order_id, p.product_name as product_id, pop.order_quantity, pop.price, pop.total_amount, pop.promotion_price, pop.promotion_plan_id, pop.volume_discount, pop.volume_discount_percent, pop.item_discount_percent, pop.item_discount_amount, pop.exclude, pop.delete_flag from pre_order_product as pop left join product as p on pop.product_id = p.id where pop.sale_order_id = :invoiceId and delete_flag = 0")
    fun getActivePreOrderProductByInvoiceIDWithName(invoiceId: String): List<PreOrderProduct>

    @Query("select * from pre_order_product where sale_order_id in (:invoiceIdList) and delete_flag = 0")
    fun getActivePreOrderProductByInvoiceIDList(invoiceIdList: List<String>): List<PreOrderProduct>
    ...
    @Query("update pre_order_product set delete_flag = 1 WHERE delete_flag = 0 and sale_order_id = :id")
    fun updateInactivePreOrderProductByID(id: String)
